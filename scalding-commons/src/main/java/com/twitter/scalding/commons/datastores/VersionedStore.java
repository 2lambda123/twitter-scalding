...
public class VersionedStore {
  ...
  public Long mostRecentVersion(boolean skipVersionSuffix, Long maxVersion)
      throws IOException {
  ...
  public long newVersion() { return System.currentTimeMillis(); }
  ...
  public List<Long> getAllVersions(boolean skipVersionSuffix)
      throws IOException {
        ...
        if (skipVersionSuffix) {
          ...
          if (!p.getName().startsWith("_")) {
            try {
              ...
              } else if (status != null && status.isDir() &&
                         getFileSystem().exists(
                             new Path(p, HADOOP_SUCCESS_FLAG))) {
                // FORCE the _SUCCESS flag into the versioned store directory.
                ret.add(validateAndGetVersion(p.toString() +
                                              FINISHED_VERSION_SUFFIX));
              }
  ...
  /**
   * The path to the hadoop-created success flag file which may or may not
   * exist
   */
  ...
  private Path normalizePath(String p) { return new Path(p).makeQualified(fs); }
  ...
  private long validateAndGetVersion(String path) {
    ...
    if (!normalizePath(path).getParent().equals(normalizePath(root))) {
      throw new RuntimeException(
          path + " " + parent +
          " is not part of the versioned store located at " + root);
    }
    ...
    try {
      ...
      if (status == null || !status.isDir())
        throw new RuntimeException(versionPath +
                                   " is not a valid version subfolder");
    } catch (IOException e) {
  ...
  public Long parseVersion(String path) {
    ...
    if (name.endsWith(FINISHED_VERSION_SUFFIX)) {
      name =
          name.substring(0, name.length() - FINISHED_VERSION_SUFFIX.length());
    }
    try {
  ...
  private void mkdirs(String path) throws IOException {
    if (fs instanceof LocalFileSystem)
      ...
      try {
        ...
        throw new RuntimeException(
            "Root directory doesn't exist, and user doesn't have the permissions "
                + "to create" + path + ".",
            e);
      }
    }
